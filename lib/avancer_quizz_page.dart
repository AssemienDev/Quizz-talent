...
class AvancerQuizzPage extends StatefulWidget {


  @override
  AvancerQuizzPageState createState() => AvancerQuizzPageState();
}

class AvancerQuizzPageState extends State<AvancerQuizzPage> with SingleTickerProviderStateMixin{
  ...
  Future init() async {
    ...
    int? ancScore = preferences.getInt('avancer');
  ...
  List<Question> questions = DatasNormale().listeQuestions;
  int index = 0;
  int score = 0;
  Timer? timer;
  int i=0;
      ...
      setState(() {
        if(seconds <= 0){
          ...
          if(this.volume == "activer") {
            playAudioFromAsset("difficile.mp3");
          }
        }
      });
    });
  ...
  Future<void> showResult() async{
    ...
    Globals.avancer = score;

    if(ancScore != null){
      if(ancScore! < score){
        preferences.setInt('avancer', score);
      }
    }else{
      preferences.setInt('avancer', score);
    }
