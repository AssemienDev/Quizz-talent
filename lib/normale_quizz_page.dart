...
class NormaleQuizzPage extends StatefulWidget {


  @override
  NormaleQuizzPageState createState() => NormaleQuizzPageState();
}

class NormaleQuizzPageState extends State<NormaleQuizzPage> with SingleTickerProviderStateMixin{
  ...
  Future init() async {
    ...
    int? ancScore = preferences.getInt('normale');
  ...
  static const maxSeconds = 16;
  int seconds = maxSeconds;
  List<Question> questions = DatasAvancer().listeQuestions;
  int index = 0;
  int score = 0;
  Timer? timer;
  int i=0;
      ...
      setState(() {
        if(seconds <= 0){
          ...
          if(this.volume == "activer") {
            playAudioFromAsset("normale.mp3");
          }
        }
      });
    });
                      ...
                      AnimatedBuilder(
                        ...
                        builder: (context, child) {
                          return Opacity(
                            ...
                            child: TextWithStyle(data: question.question, size: 18, weight: FontWeight.bold, color: Colors.white,),
                          );
                        },
                      ),
                      AnimatedBuilder(
        ...
        builder: (BuildContext contex){
          return SimpleDialog(
            ...
            children: [
              Image.asset(path),
              Center(
                child: TextWithStyle(data: (bonne)? " " : question.explication, size: 17,),
              ),
  ...
  Future<void> showResult() async{
    ...
    Globals.normale = score;

    if(ancScore != null){
      if(ancScore! < score){
        preferences.setInt('normale', score);
      }
    }else{
      preferences.setInt('normale', score);
    }
